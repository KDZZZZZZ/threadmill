"""Generate a browser check of follow-up message/activity ordering.

Usage: python3 test/webui-message-order-check.py /tmp/order-check/index.html
Serve the output directory over HTTP and open it in a browser. The shipped UI
receives mocked HTTP/SSE; the check never sends a live project request.
"""

from pathlib import Path
import sys

source = (Path(__file__).resolve().parents[1] / 'docs/webui-demo.html').read_text()
fixture = r'''
<script>
(() => {
 ...
 let sequence=0;
 const snapshot=()=>({project,messages:{items:messages},agents:{items:[]},graph,reasoning});
 window.fetch=async path=>new Response(JSON.stringify(path==='/healthz'?{service:'threadmill-webui'}:path==='/api/v1/projects'?{items:[project]}:path.endsWith('/agents')?{items:[]}:path.endsWith('/graph')?graph:project),{headers:{'Content-Type':'application/json'}});
