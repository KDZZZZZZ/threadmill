...
(() => {
 ...
 const snapshot=()=>({project,messages:{items:[]},agents,graph});
 window.fetch=async path=>new Response(JSON.stringify(path==='/healthz'?{service:'threadmill-webui'}:path==='/api/v1/projects'?{items:[project]}:path.endsWith('/messages')?{items:[]}:path.endsWith('/agents')?{items:agents}:path.endsWith('/graph')?graph:project),{status:200,headers:{'Content-Type':'application/json'}});
