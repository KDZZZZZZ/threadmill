...
| 指标卡 | Threadmill | Pi 原生路径（无 OverlayFS） | Threadmill 效果 |
| --- | ---: | ---: | ---: |
| **建议稳定协调图宽度** | **384 Agent** | 384 Agent 已饱和 | **稳定运行** |
| **实测有效峰值宽度** | **448 Agent** | 未形成有效峰值 | **明显领先** |
| **384 Agent 同宽吞吐** | **761 commands/s** | 约 324 commands/s | **约 2.35×** |
| **384 Agent 总耗时** | **15.71 s** | 37.09 s | **降低 58%** |
| **384 Agent 文件写 P95** | **98 ms** | 1.52–1.54 s | **约低 15.5×** |

上表使用 Pi 原生工具路径和共享 cwd，反映 Pi 不额外挂 OverlayFS 时的实际运行能力；384 Agent 已进入饱和。外置 OverlayFS 作为补充安全对照保留在详细结果表中，但它不是 Pi 原生 runtime 能力。
