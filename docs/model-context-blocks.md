...
当前默认角色、工具和 hook 配置在 [`threadmill.yaml`](../threadmill.yaml) 的 `agents` 段。`prompts.default` 只有在某个角色没有专用 `system_prompt` 时才会作为回退，不会与专用 prompt 自动拼接。
...
现状下，记忆图或协调图任何一个字节变化，都会作废其后全部 ReAct 历史的缓存。manager 最严重：`Snapshot.Revision`/`Executing` 逐请求变动，且 `coordination_replacePending` 会在同一 ReAct 轮内改图，导致同一轮的连续工具调用互相打掉缓存。目标布局已按 Phase 2 落地：C0+tools 构成静态前缀，紧跟 append-only 的历史；C3/C6 移到历史之后。
...
| `organize_subgraph` 失败 | requester 不追加订阅；目标空子图可能已经写入图 | 失败不会自动回滚已创建的 `sg-q-N`，所以可能留下“存在但无人订阅”的空子图。 |
...
| fork / merge | 图快照和订阅选择器分开处理 | `Fork` 只复制图，不复制另一个 Loop 的订阅；`Merge` 只合并图，不把 child 的动态订阅传播给 parent。 |

没有单独的“取消一个订阅”操作；要取消动态订阅只能用 `SetSubscribedSubgraphs` 替换列表。未知子图 ID 可以留在列表里；在正常无悬空归属的图中，`NodesInSubgraphs` 会忽略它。
...
| task sink 首次注册或协调图改变 | `SetTaskSink`、`ReplacePending`、`coordination_provideHelp` 触发 `ProjectManagerTaskInfos` | 对非空 Info upsert `task-info-<id>`；必要时新增 `task-user-input-<id>` 到 `system-task-sources` | `task-info-*` 进入 manager 固定子图；`system-task-sources` 默认不在 manager 固定订阅中，主要供 root 装配时复制到 package。相同 ID 的 Task Info 是替换，不是无界追加；已开始/已完成 task 的 info 变更会被协调图拒绝。 |
...
| child join 回 parent | `joinIncoming` 等 child 完成后调用 `Store.Merge(child, parent)` | child 相对 fork 基线的新节点、子图和边以 additive 方式并入；同 ID 同 statement 只并集归属并保留 parent 的其他字段，冲突 ID 重映射 | child 对继承节点的更新和删除不会覆盖 parent；child 的动态订阅也不传播。merge 在目标角色下一次 Ask 前执行，所以目标只有已订阅相关子图时才看到合入节点。 |
...
| planner | `<task-id>-package` | package 装配、同 task 其他角色/child 的共享图提交、planner 自己的 organize/compact | executor/verifier 的动态订阅列表和未归属 compact 节点不会自动进入。 |
...
| subgraph organizer | 默认无固定/自动注入的记忆订阅 | 它在同一 env 上执行查询工具和 `memory_apply`；深度整理也可能写图 | 不会自动收到整张图的 SystemPrompt；当前整理 query、子图目录和候选节点由用户消息/工具结果提供。 |
...
下列变化通常**不**改变普通记忆文本，但会改变 organizer/`memory_*` 请求，不能把它们误认为完全无关：`SourceRefs`、`CreatorAgentID`、`SupersededBy`、边、未订阅节点、子图 `name/summary/kind/revision` 以及无 statement 节点。
...
运行时实例 ID 为 `manager`。manager 的正常 SystemPrompt 拼接顺序是：角色提示 → manager 记忆 → 协调图 →（可选）压力提醒。
...
| C5 | `M-10 协调工具结果` | `coordination_replacePending` / `coordination_provideHelp` 返回的完整图快照；`organize_subgraph` 返回的新子图 JSON。 |
...
| 等级 | 块 | 实际内容 |
| --- | --- | --- |
| C0 | `P-01 角色系统提示` | 调查规则、IPD/拆分规则、Help Executor 计划格式、执行计划、验收矩阵和风险输出格式。 |
...
| C5 | `P-11 organize 结果` | 新子图 JSON；整理 Agent 的内部对话不会直接复制进 planner。 |
...
| C0/C1 | `O-02 工具 schema` | `memory_neighbors`、`memory_subgraphs_of`、`memory_sources_of`、`memory_nodes_in`、`memory_add_to_subgraph`、`memory_apply`、`memory_expand`、`memory_collapse`、`memory_drop_from_context`。 |
| C0 | `O-03 organize_query 规则` | 查询是数据、选择最小相关节点、过滤 superseded/outdated、不得编造 ID 等固定规则。它会嵌入整理用户消息。 |
| C3 | `O-04 当前子图目录` | `已有子图`列表，每项包含 `id`、`kind`、`name`。 |
| C3 | `O-05 候选节点列表` | 按 query 匹配出的节点 ID 和 statement；普通 `organizeQuery` 路径没有整体长度截断。 |
| C5 | `O-06 当前整理 query` | 调用方 query、目标子图 ID、整理指令；目标 ID 是递增生成的 `sg-q-N`。 |
| C5 | `O-07 普通整理用户消息` | query、目标、子图目录和候选节点的完整组合。 |
| C5 | `O-08 深度整理用户消息` | 达到阈值时的全图节点清单：ID、kind/status、创建者、statement；单节点和整体都有截断。 |
...
| C6 | `O-13 压力提醒` | 默认 organizer 唯一启用的动态 SystemPrompt hook；超过阈值才追加。 |
