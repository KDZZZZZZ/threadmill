...
| Coordination Graph | Reproducible bug fixes, invariant checks, cancellation and recovery fixes, explicit Join session lifecycle, manager-selected checkpoint publication, and optimizations that preserve current semantics | Keep Root, Spawn, Join, Help, Planner → Executor → Verifier, and existing edge semantics fixed. `coordination_orchestrate` is one Manager-facing facade over separate pending-graph and help-request state machines; requester roles only submit advice through `coordination_requestHelp`. A Join only offers isolated candidates; it never applies files automatically. Completion archives each task independently; publication renders one snapshot onto the display surface and moves a durable published-task reference to it. It never merges candidates, never consumes one, and never requires a quiescent graph, so any completed checkpoint may be rendered at any time and an earlier one may be rendered again. Do not add roles, edge kinds, graph tools, or scheduling semantics without explicit human approval |
...
| Verifier | Prompt, context, evidence collection, verdict format, and model parameters | May request independent evidence help, create disposable validation side effects, and consume Join candidates as evidence. Must not repair the persistent implementation or schedule follow-up work; defects belong in the report |
...
| Memory Graph | Storage, fork and merge, retrieval, deduplication, compaction, and bounded organization | 允许底图保留冗余、过期和局部冲突，不以全局整洁为目标。读取时必须能低成本整理出当前任务的最小相关视图；仍相关的冲突须保留来源，不能靠删除历史掩盖。记忆图使用可配置的节点数软上限；达到上限时明确提醒整理 Agent 做全图整理，而不是限制其思考步数。整理 Agent 只在高价值触发点运行，输入和输出必须有界，继承调用方取消，不另设思考时限；不得访问文件、命令或协调图，并以确定性校验和无操作回退保护结果。在一次 organize 查询范围内，整理 Agent 还负责维护子图说明（准入内容与适用范围）并调整**发起该查询的** Agent 的动态订阅；两者都只在该查询收尾时生效，查询失败即零变化，固定订阅与任务启动包结构上不可取消 |
| VFS | Internal algorithms for snapshots, overlays, materialization, immutable task archives, transactional final publication, candidate change inspection, selective safe/replace apply, indexing, caching, cleanup, and recovery | Preserve isolation and visible file semantics. A failed publication keeps the selected archive retryable and must not leave a claimed commit. Candidate inspection is read-only; safe apply is all-or-none on conflicts; replace requires an explicit role decision. Do not escape configured roots, expose internal storage to Agents, or trade correctness for a fast path |
...
| --- | --- | --- |
| Manager | Request closure, correct task creation, boundary compliance, evidence-based snapshot selection, recovery, latency, and token cost | Manager model and tool events; coordination-tool calls and errors, including selected `task_id` and `published`; `pending`; `task_running`; task reports; Manager TTFT, P50/P95, and tokens |
| Coordination Graph | Graph invariants, explicit candidate disposition, exactly-once Join completion, terminal-state correctness, selected-snapshot publication, cancellation, recovery, and scheduling latency | Graph revision, tasks and edges; progress `Outputs`, `Joins`, `Merged`, and `Prepared`; candidate applied/discarded state; selected publication task and result; task start/end events; active/done/failed/canceled counts; stuck-active duration |
...
`done` means that a task flow reached a terminal state; it neither replaces the Verifier verdict nor means its snapshot has been published. The real project changes only after a successful manager-selected publication.

The project directory is a display surface, not the substrate environments read. Environments read an immutable floor cloned when the session adopts the project, so publication renders a checkpoint for the user without moving anything underneath running work. The floor is re-adopted only when the project changed outside Threadmill between sessions.
