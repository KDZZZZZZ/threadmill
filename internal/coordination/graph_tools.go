...
func (t graphTool) Definition() agenttool.Definition {
	return agenttool.Definition{
		...
		Description: "提交协调图尚未开始部分的完整期望态。图执行中可以追加后续 root，或新增、删除、调整尚未开始节点上的辅助分支；已经开始的 task info 和节点关联边会被拒绝。roots 按序号对齐且只可追加；新增或尚未开始的 root 与 spawn 必须有非空白 info；已完成 root 的 info 仅作位置占位并被忽略。新增 root 从前一个 root 的 task 持久环境（记忆和文件）fork；辅助任务只能写 spawns，不能写成额外 root。from/join 必须是当前图或上次工具结果中的完整节点 ID（如 task-1:planner），禁止占位符。新 root 需要 spawn 时分两次：先只提交 roots 取得真实 ID，再原样保留 roots 并提交完整 spawns。失败时图不变。root 按出现顺序串行执行，同时 active 的 root 只是排队，不会并行；唯一的并行面是 task 内部经拆分请求物化的辅助任务。root 的 run_policy 可写 enabled 或 held：held 表示该 root 留在队列里但不启动，用来表达激活条件尚未满足的后续 wave；队头 held 会挡住其后全部 root；省略该字段保持现状，新建 root 默认 enabled；已完成 root 的 run_policy 与 info 一样被忽略。",
		InputSchema: json.RawMessage(`{"type":"object","properties":{"roots":{"type":"array","minItems":1,"description":"全部根任务，必须保留现有顺序且只可追加；新增或尚未开始 root 的 info 必须非空白，已完成 root 的 info 仅作位置占位并被忽略。辅助分支不要放这里。","items":{"type":"object","properties":{"info":{"type":"string"},"run_policy":{"type":"string","enum":["enabled","held"],"description":"enabled 立即排队执行；held 留在队列里不启动并挡住其后全部 root。省略保持现状，新建 root 默认 enabled。"}},"required":["info"],"additionalProperties":false}},"spawns":{"type":"array","description":"完整 spawn/join 期望集；执行中仅可改变尚未开始节点上的分支。新根先单独创建并从工具结果取得真实节点 ID。","items":{"type":"object","properties":{"from":{"type":"string","description":"尚未开始的真实来源节点 ID，例如 task-1:executor。"},"join":{"type":"string","description":"同一任务树中尚未开始的真实汇合节点 ID，例如 task-1:verifier。"},"info":{"type":"string","minLength":1,"description":"非空白的子任务目标。"}},"required":["from","join","info"],"additionalProperties":false}}},"required":["roots"],"additionalProperties":false}`),
	}
}
